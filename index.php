 ...
 #read all data
    ...
    if (is_numeric($varKeyword)){ //probaby afm
        if (strlen(utf8_decode($varKeyword)) <=6 ) {
             $search->getAll('',$varKeyword,DbPath,couchUser,couchPass,solrPath,altNamesSolrCore );	
        }
        else {
            $search->getAllShort('*',$varKeyword,DbPath,couchUser,couchPass,solrPath,altNamesSolrCore);	
        }
    }
    else { #name
        ...
        if(strlen($varKeyword) != mb_strlen($varKeyword, 'utf-8')){  #greek found
           if (count($words) === 1){
               if (strlen(utf8_decode($varKeyword)) <= 4 ){ # greek  like
                    $search->getAllGreek('*',$varKeyword,DbPath,couchUser,couchPass,solrPath,altNamesSolrCore); 
               }
               else { # exact, fuzzy and then like
                   $search->getAllGreek('',$varKeyword,DbPath,couchUser,couchPass,solrPath,altNamesSolrCore);  	
                   $search->getAllGreek('~0.75', $varKeyword, DbPath,couchUser,couchPass,solrPath,altNamesSolrCore);
                   $search->getAllGreek('*',$varKeyword,DbPath,couchUser,couchPass,solrPath,altNamesSolrCore);
               }
           }
           else{
               if (count($words) > 1) {
                   ...
                   $search->getAllGreek('',$varKeyword,DbPath,couchUser,couchPass,solrPath,altNamesSolrCore);
                   $search->getAllGreek('',$term12,DbPath,couchUser,couchPass,solrPath,altNamesSolrCore);
                   if (strlen(utf8_decode($term1)) <=4 ){	
                        $search->getAllGreek('*',$term1,DbPath,couchUser,couchPass,solrPath,altNamesSolrCore);
                   }
                   else {
                        $search->getAllGreek('~0.75',$term1,DbPath,couchUser,couchPass,solrPath,altNamesSolrCore);	
                        $search->getAllGreek('*',$term1,DbPath,couchUser,couchPass,solrPath,altNamesSolrCore);	
                   }
                   if (strlen(utf8_decode($term2)) <=4 ){	
                       $search->getAllGreek('',$term2,DbPath,couchUser,couchPass,solrPath,altNamesSolrCore);	
                   }
                   else {
                       $search->getAllGreek('~0.75',$term2,DbPath,couchUser,couchPass,solrPath,altNamesSolrCore);	
                       $search->getAllGreek('*',$term2,DbPath,couchUser,couchPass,solrPath,altNamesSolrCore);
                   }
               }
           }
            ...
            if (count($words) == 1){
                if (strlen(utf8_decode($varKeyword)) <=4 ) {
                    $search->getAll('*',$varKeyword,DbPath,couchUser,couchPass,solrPath,altNamesSolrCore);	
                     ...
                     $search->getAll('',$varKeyword,DbPath,couchUser,couchPass,solrPath,altNamesSolrCore);
                     $search->getAll('~0.75',$varKeyword,DbPath,couchUser,couchPass,solrPath,altNamesSolrCore);	
                     $search->getAll('*',$varKeyword,DbPath,couchUser,couchPass,solrPath,altNamesSolrCore);		
                ...
                if (count($words) > 1) {
                    ...
                    $search->getAll('',$varKeyword,DbPath,couchUser,couchPass,solrPath,altNamesSolrCore);
                    $search->getAll('',$term12,DbPath,couchUser,couchPass,solrPath,altNamesSolrCore);
                    if (strlen(utf8_decode($term1)) <=4 ){
                        $search->getAll('*',$term1,DbPath,couchUser,couchPass,solrPath,altNamesSolrCore);	  
                    }
                    else {
                        $search->getAll('~0.75',$term1,DbPath,couchUser,couchPass,solrPath,altNamesSolrCore);
                        $search->getAll('*',$term1,DbPath,couchUser,couchPass,solrPath,altNamesSolrCore);
                    }
                    if (strlen(utf8_decode($term2)) <=4 ){
                          $search->getAll('',$term2,DbPath,couchUser,couchPass,solrPath,altNamesSolrCore);
                    }
                    else {
                        $search->getAll('~0.75',$term2,DbPath,couchUser,couchPass,solrPath,altNamesSolrCore);
                        $search->getAll('*',$term2,DbPath,couchUser,couchPass,solrPath,altNamesSolrCore);
                    }
